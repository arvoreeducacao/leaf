...
App iOS e Android do Leaf. É uma WebView do Leaf num shell Expo: o site roda como está, e o app cuida só do que a web não faz no celular.

## O que o shell faz

- **Login pelo navegador do sistema.** Provedor de SSO costuma recusar WebView. A tela de login nativa abre `ASWebAuthenticationSession` (iOS) ou Custom Tabs (Android) no fluxo de `signIn.social` do Leaf; o Leaf devolve a sessão em `app.leaf://?cookie=…` (plugin Expo do better-auth) e o app guarda no SecureStore.
...
- **Domínio único.** Navegação de topo para fora de `leaf.example.com` abre no navegador do sistema. No iOS, `WKAppBoundDomains` prende a WebView ao domínio e libera o service worker do Leaf (offline).
